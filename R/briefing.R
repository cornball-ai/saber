...
briefing <- function(project = NULL,
                     ...
                     scan_dir = path.expand("~"), max_memory_lines = 30L) {
    if (is.null(project)) {
...
briefing_ontology <- function(project, vault_path) {
    ...
    if (is.na(term_id)) {
        return(sprintf("_%s is not in the ontology._", project))
    }
    ...
    if (length(isa) > 0L) {
        lines <- c(lines,
                   sprintf("- **is_a**: %s", paste(isa, collapse = ", ")))
    }
    ...
    if (length(partof) > 0L) {
        lines <- c(lines,
                   sprintf("- **part_of**: %s", paste(partof, collapse = ", ")))
    }
    ...
    if (length(wraps) > 0L) {
        lines <- c(lines,
                   sprintf("- **wraps**: %s", paste(wraps, collapse = ", ")))
    }
    ...
    if (length(uses) > 0L) {
        ...
        lines <- c(lines,
                   sprintf("- **uses**: %s", paste(deps, collapse = ", ")))
    }
...
briefing_siblings <- function(project, vault_path) {
    ...
    for (p in parents) {
        ...
        if (length(these) > 10L) {
            these <- c(these[1:10],
                       sprintf("... +%d more", length(these) - 10L))
        }
        lines <- c(lines,
                   sprintf("- **%s**: %s", p, paste(these, collapse = ", ")))
    }
    lines
}
