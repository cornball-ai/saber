...
write_claude_instructions <- function(outfile, index_path) {
    ...
    instructions <- c("# basalt: Project Ontology", "",
                      ...
                      "Projects are auto-registered as terms. R package dependencies are", "auto-inferred as `uses` relations.",
                      "", "## Quick reference", "", "```r",
                      "library(basalt)", "", "# Check what's indexed",
                      sprintf("basalt::status(vault_path = \"%s\")", index_path), "",
                      "# Query relationships",
                      sprintf("basalt::query(\"torch\", \"uses\", \"descendants\", vault_path = \"%s\")",
                              index_path), sprintf("basalt::query(\"whisper\", \"uses\", \"ancestors\", vault_path = \"%s\")",
            index_path),
                      ...
                      "```r", "# Add terms",
                      sprintf("basalt::add(terms = c(\"transformer\", \"attention\"), vault_path = \"%s\")",
                              index_path), "",
                      ...
                      "  ),", sprintf("  vault_path = \"%s\"", index_path),
                      ")", "```", "",
                      sprintf("Additions are written to `%s` as markdown",
                              file.path(dirname(index_path), "annotations")), "files so they persist across re-indexes.",
                      "", "## Shell usage", "", "```bash",
                      sprintf("r -e 'basalt::query(\"torch\", \"uses\", \"descendants\", vault_path = \"%s\")'",
                              index_path), "r -e 'basalt::startup()'",
                      sprintf("r -e 'basalt::status(vault_path = \"%s\")'", index_path), "```",
                      "", "## The suggest/confirm loop", "",
                      "basalt can propose typed relations from untyped wikilinks:", "",
                      "```r", sprintf("basalt::suggest(\"%s\")", index_path),
                      "```", "",
                      ...
                      sprintf("1. Edit the annotation file in %s",
                              file.path(dirname(index_path), "annotations")), "2. Run `basalt::startup()` to pick up the change",
                      "3. Do NOT manually patch the index files", "",
                      "## Promoting terms", "", "```r",
                      sprintf("basalt::promote(\"term_name\", \"%s\")", index_path), "```",
                      "", "Only do this when Troy asks. Never auto-promote.",
                      "", "## OBO export", "", "```r",
                      sprintf("basalt::emit_obo(vault_path = \"%s\", outfile = \"ontology.obo\")",
                              index_path), "```")
